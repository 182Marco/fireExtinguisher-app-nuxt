...
export const getters = {
  cartItemsLenght: state => state.cartItems.length,
  cartNoDuplicate: state =>
    state.cartItems.filter(
      (e, i, a) => i === a.findIndex(el => el.id === e.id)
    ),
  elInCartLength: state => id => state.cartItems.filter(e => (e.id = id)).length
};

export const mutations = {
  addItem(state, id) {
    state.cartItems.push(state.products.find(e => e.id == id));
  },
  removeEl(state, id) {
    const i = state.cartItems.findIndex(e => e.id == id);
